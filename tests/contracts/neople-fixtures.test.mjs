...
const fixtureDirectory = path.resolve(testDirectory, '../../docs/validation/neople/fixtures');
...
async function fixture(filename) {
  try {
    ...
    if (error?.code === 'ENOENT') {
      assert.fail(`${filename}이 없습니다. 먼저 capture-contract-fixtures.mjs를 실행하세요.`);
    }
    throw error;
  }
}
...
test('정확한 MVP 아이템 하나와 itemId를 확정한다', async () => {
  ...
  for (const key of itemSearchKeys) {
    const expectedType = ['itemAvailableLevel', 'fame'].includes(key) ? 'number' : 'string';
...
test('등록 매물 기본 응답은 문서상 최대 400건이며 auctionNo 오름차순이다', async () => {
  ...
  for (let index = 0; index < auctionRows.length; index += 1) {
    ...
    const previous = comparableInteger(auctionRows[index - 1].auctionNo, 'auctionNo');
...
test('등록 매물은 unitPrice 오름차순으로 요청할 수 있다', async () => {
  ...
  for (let index = 1; index < auctionRows.length; index += 1) {
    const previous = comparableInteger(auctionRows[index - 1].unitPrice, 'unitPrice');
    const current = comparableInteger(auctionRows[index].unitPrice, 'unitPrice');
...
test('체결 내역 응답은 문서상 최대 100건이다', async () => {
  ...
  for (let index = 0; index < soldRows.length; index += 1) {
    ...
    const fingerprint = [row.soldDate, row.itemId, row.unitPrice, row.count].join('|');
    assert.ok(
