...
if (!API_KEY) {
  throw new Error('NEOPLE_API_KEY가 설정되지 않았습니다. 키는 환경변수로만 전달하세요.');
}

const scriptDirectory = path.dirname(fileURLToPath(import.meta.url));
const repositoryRoot = path.resolve(scriptDirectory, '../..');
const fixtureDirectory = path.join(repositoryRoot, 'docs', 'validation', 'neople', 'fixtures');
...
async function writeFixture(filename, value) {
  ...
  await writeFile(path.join(fixtureDirectory, filename), `${serialized}\n`, 'utf8');
}
...
const auctionPriceAscending = await fetchJson('/df/auction', auctionPriceParameters);
